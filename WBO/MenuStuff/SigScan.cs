    ...
    internal class SigScan
    {
        ...
        public static async Task ConnectWorld()
        {
            if (!IsWorldConnectionAttempting)
            {
                ...
                var GameArrayHook = await MemLib.AoBScan(MemLib.mProc.MainModule.BaseAddress.ToInt64(),
                MemLib.mProc.MainModule.BaseAddress.ToInt64() + MemLib.mProc.MainModule.ModuleMemorySize,
                "48 8B 05 ? ? ? ? 48 8B D9 49 8B C8 48 8B B8", true, true, true, "");
               ...
               "48 8B D9 48 8D 0D ? ? ? ? BA ? ? ? 00 E8 ? ? ? ? 89", true, true, true, "");
                ...
                TimeBPBaseAddress = (Address + MemLib.ReadInt((Address + 13).ToString("X2")) + 20 + 0x4ED50F76).ToString("X2");


                Debug.WriteLine(


              "TimeBPBaseAddress: " + TimeBPBaseAddress
              + "\n" + "\n" +
              "TimeBPAssemblyAddress: " + TimeBPAssemblyAddress
              + "\n" +
              "Global_: " + Global_
              );
